...
create_subsample_rf_input <- function(country, runs, samples){
  ...
  run_variables <- c()
  for (i in runs){
    variables <- read.csv(paste0('/Users/SBK/Desktop/Tez/Practical/data/', i,'.csv'))$names
    run_variables <- c(run_variables, variables)
  }
  selected_variables <- unique(run_variables)
  print(country)
  
  stationInfo <- read.csv(paste0('/Users/SBK/Desktop/Tez/Practical/data/stationLatLon_selected_', country,'.csv'))
  for (i in 1:samples){
    print(paste0('subsample: ', print(i)))
    subsample <- i
    
    outputDir <- paste0('/Users/SBK/Desktop/Tez/Practical/RF/0_rf_input/subsample_',subsample,'_',country,'/')
    print(outputDir)
    ...
    source('/Users/SBK/Desktop/Tez/Practical/data/R/4_randomForest/fun_2_0_subsample_train_test.R')
    ...
    for (run in runs){
      selected_variables <- read.csv(paste0('/Users/SBK/Desktop/Tez/Practical/data/', run,'.csv'))$names
...
filePathPreds <- paste0('/Users/SBK/Desktop/Tez/Practical/data/predictors/pcr_allpredictors/')
fileListPreds <- list.files(filePathPreds, pattern='.csv')
filenames <- paste0(filePathPreds, fileListPreds)

#countries <- c('US', 'AU', 'CA', 'all')
countries <- c('all')
...
for (country in countries) {
  runs <- c('predictors_pcr', 'predictors_pcr_sat_add','predictors_pcr_sat','predictors_sat_meteo',
            ...
            'predictors_pcr_sat_E_add','predictors_pcr_sat_E')
  create_subsample_rf_input(country, runs, samples)
}
...
for (country in countries){
  for (subsample in 1:samples){
    ...
    for (run in runs){
      selected_variables <- read.csv(paste0('/Users/SBK/Desktop/Tez/Practical/data/', run,'.csv'))$names
      train_stations <- read.csv(paste0('/Users/SBK/Desktop/Tez/Practical/RF/0_rf_input/subsample_',
