...
outputDir <- 'C:/Users/sbusr/Desktop/Practical/data/vizZA/varImportance/'
  ...
  for (c in regions){
    ...
    if (c == 'ZA'){
      plotData$predictorType <- factor(plotData$predictorType, levels = c('State variables', 'Satellite products', 'Meteorological input',
                                                                          'Static variables'))
      colors <- c('#008080', "#e66464",'#00CC99', "#E69F00")
      title = 'Brazil'
      ...
      colors <- c("green",'darkgreen', "blue", '#37e1e1', "red", '#e66464') #"#1f78b4"
    }
    ...
    ggsave(paste0('C:/Users/sbusr/Desktop/Practical/data/vizZA/varImportance/', c,'_',setup[i], '.pdf'),viPlot,height=20, width=20, units='in', dpi=600) 
